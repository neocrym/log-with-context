    ...
    def test_inline_extra(self):
        ...
        for method_name in LOG_METHOD_NAMES:
            with self.subTest(name=f"logger.{method_name}"):
                ...
                self.gb(method_name).assert_called_with(
                    "%s", method_name, extra=dict(hello=method_name)
                )
